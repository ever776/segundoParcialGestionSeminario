    ...
    <body>
        ...
        <form action="ParticipanteController" method="post" >
            ...
            <div class="mb-3">
                ...
                <label for="seminario" class="form-label">Seminario</label>                                    
                <select name="id_seminario" class="form-label">
                    <option value="">-- Seleccione --</option>
                    <c:forEach var="item" items="${lista_seminarios}">                        
                    <option value="${item.id}" <c:if test="${participante.id_seminario == item.id}">
                                    selected
                ...
                name="confirmado" value="1" <c:if test="${participante.confimado == 1}">
                                    checked
                                </c:if>>
               <label for="contactChoice2">SI</label>
               <input type="radio" id="contactChoice2"
                name="confirmado" value="0" <c:if test="${participante.confimado == 0}">
                                    checked
                                </c:if>>
